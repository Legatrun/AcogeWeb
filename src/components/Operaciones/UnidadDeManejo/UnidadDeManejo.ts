...
export default class AdmUnidadDeManejoComponent extends Vue {
	...
	private Insertar(): void {
		this.unidaddemanejo = new services.clase_unidaddemanejo();
		this.operacion = 'Insert';
		this.dialog = true;
	}
	private Grabar() {
		if (this.operacion === 'Update') {
			new services.Operaciones().Actualizar(this.WebApi.ws_unidaddemanejo_Actualizar, this.unidaddemanejo)
			.then((result) => {
		...
		new services.Operaciones().Insertar(this.WebApi.ws_unidaddemanejo_Insertar, this.unidaddemanejo)
		.then((result) => {
			if (result.data.error === 0) {
	...
	private Actualizar(data: services.clase_unidaddemanejo): void {
		this.unidaddemanejo = data;
	...
	private Eliminar(data: services.clase_unidaddemanejo): void {
		swal.fire({
			...
			new services.Operaciones().Eliminar(this.WebApi.ws_unidaddemanejo_Eliminar, data)
				.then((result) => {
				if (result.data.error === 0) {
					swal.fire({
					type: 'success',
