	...
	<v-card>
		...
		<v-data-table 	style="padding: 5px"
			...
			<template v-slot:top>
				<v-tooltip bottom>
					<template v-slot:activator="{ on }">
						<v-btn color="accent" v-on="on" @click="Insertar()"><v-icon left>mdi-plus</v-icon>Añadir Cuenta de Presupuesto</v-btn>
					</template>
				</v-tooltip>
			</template>
					...
					<v-card-text>
						<v-layout wrap>
							...
							<v-flex sm6 style="padding: 3px">
								<v-col cols="4" sm="5">
									<p class="text-sm-left"><b>Balance de Cuenta: </b></p> <v-switch  v-model="ctaspresup.balancecuenta" color="success"  :label="`Estado: ${ctaspresup.balancecuenta ? 'Si' : 'No'}`"> </v-switch>
									</v-col>
							</v-flex>
							
							<v-flex sm6 style="padding: 3px">
								<v-col cols="4" sm="5">
									<p class="text-sm-left"><b>Cuenta de Asiento: </b></p> <v-switch  v-model="ctaspresup.cuentaasiento" color="success"  :label="`Estado: ${ctaspresup.cuentaasiento ? 'Si' : 'No'}`"> </v-switch>
									</v-col>
							</v-flex>
							
							<v-flex sm6 style="padding: 3px">
								<v-col cols="4" sm="5">
									<p class="text-sm-left"><b>Grabado: </b></p> <v-switch  v-model="ctaspresup.grabado" color="success"  :label="`Estado: ${ctaspresup.grabado ? 'Si' : 'No'}`"> </v-switch>
									</v-col>
							</v-flex>
						</v-layout>
					</v-card-text>
