	...
	<v-card>
		...
		<v-data-table 	style="padding: 5px"
			...
			<template v-slot:top>
				<v-tooltip bottom>
					<template v-slot:activator="{ on }">
						<v-btn color="botonCrear" dark v-on="on" @click="Insertar()"><v-icon left>mdi-plus</v-icon>Añadir Registro de Cajas</v-btn>
					</template>
					<span>Adicionar nuevo registro de Caja</span>
				</v-tooltip>
				<v-tooltip bottom>
					<template v-slot:activator="{ on }">
						<v-btn color="botonAdicional" outlined v-on="on" @click="newMoneda()">Añadir Registro Monedas</v-btn>
					</template>
				...
				<v-form ref="form" style="padding:10px" v-model="activo">
					<v-card-text>
						<v-layout wrap>
							...
							<v-flex sm6 style="padding: 5px">
								<v-text-field v-model.number="cajas.cuenta"
											type="number"
											label="Cuenta"
											clearable
											persistent-hint
							...
							<v-flex sm6 style="padding: 5px">
								<v-text-field v-model.number="cajas.monto" type="number"
											label="Monto"
											clearable
											persistent-hint
											required
