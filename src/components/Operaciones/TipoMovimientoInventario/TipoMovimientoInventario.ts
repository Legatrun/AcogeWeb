...
export default class AdmTipoMovimientoInventarioComponent extends Vue {
	...
	private buscartipomovimientoinventario = '';
	...
	private Insertar(): void {
		this.tipomovimientoinventario = new services.clase_tipomovimientoinventario();
		this.operacion = 'Insert';
		this.dialog = true;
	}
	private Grabar() {
		if (this.operacion === 'Update') {
			new services.Operaciones().Actualizar(this.WebApi.ws_tipomovimientoinventario_Actualizar, this.tipomovimientoinventario)
			.then((result) => {
		...
		new services.Operaciones().Insertar(this.WebApi.ws_tipomovimientoinventario_Insertar, this.tipomovimientoinventario)
		.then((result) => {
			if (result.data.error === 0) {
	...
	private Actualizar(data: services.clase_tipomovimientoinventario): void {
		this.tipomovimientoinventario = data;
	...
	private Eliminar(data: services.clase_tipomovimientoinventario): void {
		swal.fire({
			...
			text: 'Eliminacion de Registro' + data.idtipomovimiento,
			type: 'warning',
			...
			new services.Operaciones().Eliminar(this.WebApi.ws_tipomovimientoinventario_Eliminar, data)
				.then((result) => {
				if (result.data.error === 0) {
					swal.fire({
					type: 'success',
