<template>
	<v-card>
		...
		<v-data-table 	style="padding: 5px"
						...
						:items-per-page="30"
						:search = "buscartipomovimientoinventario" 
						:footer-props="{
				...
				<tr>
					...
					<td>
						<v-tooltip bottom>
							...
							<span>Modificar Registro de Tipo Movimiento Inventario</span>
						</v-tooltip>
						<v-tooltip style="padding-left:10px" bottom>
							<template v-slot:activator="{ on }" >
								<v-btn color="error" v-on="on" fab small dark  @click="Eliminar(props.item)"><v-icon>delete</v-icon></v-btn>
							</template>
							<span>Eliminar Registro de Tipo Movimiento Inventario</span>
						</v-tooltip>
					</td>
				</tr>
			</template>
						...
						<v-layout wrap>
							...
							<v-flex sm6 style="padding: 5px">
								<v-text-field v-model="tipomovimientoinventario.descripcion"
											...
											required
											:rules="validacion"
											@input="tipomovimientoinventario.descripcion = updateText(tipomovimientoinventario.descripcion)">
								</v-text-field>
							</v-flex>
						</v-layout>
					</v-card-text>
...
<script src="./TipoMovimientoInventario.ts"></script>
