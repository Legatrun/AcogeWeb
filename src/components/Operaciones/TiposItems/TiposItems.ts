...
export default class AdmTiposItemsComponent extends Vue {
	...
	private Insertar(): void {
		this.tipositems = new services.clase_tipositems();
		this.operacion = 'Insert';
		this.dialog = true;
	}
	private Grabar() {
		if (this.operacion === 'Update') {
			new services.Operaciones().Actualizar(this.WebApi.ws_tipositems_Actualizar, this.tipositems)
			.then((result) => {
		...
		new services.Operaciones().Insertar(this.WebApi.ws_tipositems_Insertar, this.tipositems)
		.then((result) => {
			if (result.data.error === 0) {
	...
	private Actualizar(data: services.clase_tipositems): void {
		this.tipositems = data;
	...
	private Eliminar(data: services.clase_tipositems): void {
		swal.fire({
			...
			new services.Operaciones().Eliminar(this.WebApi.ws_tipositems_Eliminar, data)
				.then((result) => {
				if (result.data.error === 0) {
					swal.fire({
					type: 'success',
